...
-----------
dm-v2 (default):
  Documented DM Manage v2 endpoint.
  POST /2/dm_conversations/{conversation_id}/messages
  Body: {"text": "..."}
  Documented in X DM Manage API (docs.x.com/x-api/direct-messages).

chat-api:
  Documented Chat encrypted message endpoint from the XChat migration guide.
  POST /2/chat/conversations/{conversation_id}/messages
  Body: already-encrypted fields produced by chat-xdk.
  Use send_encrypted_reply() — NOT send_reply() — for this mode.
  Raises ValueError if called via send_reply() with only plaintext.

xchat-observed:
  EXPERIMENTAL: adds observed XChat extras (reply_to_dm_event_id,
  conversation_token) to the dm-v2 endpoint. Only when you have a valid
...
from xchat_bot.reply.adapter import EncryptedReplyPayload, ReplyResult
...
# Chat API encrypted message endpoint — documented in XChat migration guide.
# POST /2/chat/conversations/{conversation_id}/messages
_CHAT_MESSAGES_TEMPLATE = "https://api.x.com/2/chat/conversations/{conversation_id}/messages"
...
class XApiReplyAdapter:
    ...
    reply_mode controls the request body:
      "dm-v2" (default): documented DM Manage v2 — body is {"text": "..."}
          POST /2/dm_conversations/{conversation_id}/messages
      "chat-api": documented Chat encrypted send endpoint — body is already-encrypted
          fields from chat-xdk. Use send_encrypted_reply(), not send_reply().
          POST /2/chat/conversations/{conversation_id}/messages
    ...
    Args:
        ...
        reply_mode: "dm-v2" (default), "chat-api", or "xchat-observed".
    """

    def __init__(
        self,
        settings: AppSettings,
        reply_mode: Literal["dm-v2", "chat-api", "xchat-observed"] = "dm-v2",
    ) -> None:
    ...
    async def send_reply(
        ...
        *,
        ...
        In "chat-api" mode, this method ALWAYS returns an error. Use
        send_encrypted_reply() instead — chat-api requires already-encrypted
        payload fields produced by chat-xdk.
        ...
        """
        if self._reply_mode == "chat-api":
            return ReplyResult(
                success=False,
                error=(
                    "chat-api reply mode requires an already-encrypted payload generated by "
                    "chat-xdk; real encryption is not implemented in this starter. "
                    "Use send_encrypted_reply() with an EncryptedReplyPayload instead, "
                    "or switch to reply_mode='dm-v2' to send a plain DM."
                ),
            )
    ...
    async def send_encrypted_reply(
        self,
        conversation_id: str,
        payload: EncryptedReplyPayload,
    ) -> ReplyResult:
        """Send an already-encrypted message via the Chat API.

        Uses POST /2/chat/conversations/{conversation_id}/messages with the
        encrypted payload fields produced by chat-xdk.

        This method does NOT encrypt anything. All fields in ``payload`` must
        be produced by chat-xdk before calling this method.
        chat-xdk is pending stable public release — see docs/known-caveats.md.

        Args:
            conversation_id: The conversation to post into.
            payload: Already-encrypted payload from chat-xdk.
        ...
        url = _CHAT_MESSAGES_TEMPLATE.format(conversation_id=conversation_id)
        ...
        if payload.conversation_token is not None:
            body["conversation_token"] = payload.conversation_token
